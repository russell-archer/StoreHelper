...
struct ExtendedPurchaseInfo: Hashable {
...
struct PurchaseInfoViewModel {
    ...
    @MainActor func extendedPurchaseInfo(for nonConsumableProductId: ProductId) async -> ExtendedPurchaseInfo? {
    ...
    @MainActor func isPurchased(productId: ProductId) async -> Bool { (try? await storeHelper.isPurchased(productId: productId)) ?? false }
}
